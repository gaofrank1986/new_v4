...
!
	  REAL*4,INTENT(IN):: X,Y
        Real*4  ETI
!
         ETI=Amp*Cos(WK*( X*COS(BETA)+Y*SIN(BETA)) -W1*TimeRK)
	   ETI=RAMPF*ETI
...
!
	  ...
	  REAL*4  WKX,DUM
C
         DUM=AMP*G/W1*COSH(WK*(Z+H))/COSH(WK*H)
         WKX=WK*( X*COS(BETA)+Y*SIN(BETA))
         POXY=DUM*SIN(WKX-W1*TimeRK)
C
	   POXY=RAMPF*POXY
...
!
	  ...
	  REAL*4  WKX,DUM
C
         DUM=-AMP*G*COSH(WK*(Z+H))/COSH(WK*H)
         WKX=WK*( X*COS(BETA)+Y*SIN(BETA))
         DPOT=DUM*COS(WKX-W1*TimeRK)
C
	   DPOT=RAMPF*DPOT
...
C
	  ...
	  REAL*4,INTENT(IN):: X,Y,Z
        REAL*4,INTENT(OUT)::  DPOX,DPOY,DPOZ
...
C
         ...
         DPOX= DUM*WK*COS(BETA)*
	1		   COSH(WK*(Z+H))/COSH(WK*H)*COS(WKX-W1*TimeRK)
         DPOY= DUM*WK*SIN(BETA)*
	1		   COSH(WK*(Z+H))/COSH(WK*H)*COS(WKX-W1*TimeRK)
         DPOZ= DUM*WK*
	1		   SINH(WK*(Z+H))/COSH(WK*H)*SIN(WKX-W1*TimeRK)
!
	   DPOX=RAMPF*DPOX
	   DPOY=RAMPF*DPOY
	   DPOZ=RAMPF*DPOZ
...
C
	  ...
	  REAL*4,INTENT(IN) :: SI,ETA
	  REAL*4,INTENT(OUT):: SF(8),DSF(2,8)
C
...
C
	...
	REAL*4 XIQ(4),WIQ(4)
	REAL*4 SF(8),DSF(2,8),XJ(3,3)
	REAL*4 SITRI(3,3),ETATRI(3,3)
	REAL*4 XYZT(3,8),DXYZT(3,8)
	REAL*4 X,Y,Z,DX,DY,DZ,SI,ETA,XRHO,YRHO,AG,
	1	DET,DET1,DET2,DET3,DUM,DUM1,DUM2,DUM3 
C
C
C
...
C
     ...
     1     NODNUM.EQ.5.OR.NODNUM.EQ.7) THEN
           NOTRI=2
          ELSE
           NOTRI=3
          ENDIF
...
C
      	...
      	DET=SQRT(DET1*DET1+DET2*DET2+DET3*DET3)
C
C
      	X=0.0D0
      	Y=0.0D0
...
C
	 INTEGER,INTENT(OUT):: MSTA,MEND
       REAL*4,INTENT(OUT):: BJ4(NAA),DBJ4(NAA)
	 REAL*4,INTENT(IN):: X
...
C
	 DO 400 J=MSTA, MEND  	  
	     ...
	     I4=J+ 1
C
	   IF( ABS(B(J, KBSPL)) .LE. 1E-8) THEN
	     D=0.0D0
	   ELSE
	     D=B(J,KBSPL)/(UTJ(I2)-UTJ(I1))
	   END IF
C           
	   IF( ABS(B(J+1, KBSPL)).LE.1E-8) THEN
	     E=0.0D0
	   ELSE
...
C
      ...
      REAL*4    b(mp),u(mp,np),v(np,np),w(np),x(np)
      PARAMETER (NMAX=5000)
      DIMENSION tmp(NMAX) 
C      
      do 12 j=1,n
